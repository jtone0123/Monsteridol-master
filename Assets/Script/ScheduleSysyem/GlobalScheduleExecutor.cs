...
public class ManagedIdol
{
    ...
    public Idol idolMovementController;      // 아이돌의 이동 및 targetNavPoint 관련 로직
...
public class GlobalScheduleExecutor : MonoBehaviour
{
    ...
    IEnumerator ProcessAllIdolQueuesCoroutine()
    {
        ...
        UIManager.instance.AvailableNextTurn();
    ...
    IEnumerator ProcessSingleIdolScheduleQueueCoroutine(ManagedIdol currentManagedIdol)
    {
        ...
        while (queuePanel.transform.childCount > 0)
        {
            ...
            if (bundleSizeN > 0)
            {
                ...
                if(scheduleNavType.targetNavPoint == null)
                {
                    scheduleNavType.FindNavPoint();
                }
                if (scheduleNavType != null && scheduleNavType.targetNavPoint != null)
                {
                    Debug.Log($"아이돌 '{idolCharacter.characterName}': '{scheduleNavType.targetNavPoint.pointName}'으로 이동 시작.");

                    // 이동 전 콜백 등록 및 플래그 초기화
                    hasArrivedForCurrentSchedule = false; // 도착 플래그 재설정
                    idolMovement.OnArrivedAtTarget += arrivalCallback;
                    idolMovement.AssignScheduledTarget(scheduleNavType.targetNavPoint);
                    ...
                    {
                        Debug.Log($"아이돌 '{idolCharacter.characterName}': '{scheduleNavType.targetNavPoint.pointName}' 도착.");
                    }
                    else
                    {
                        Debug.Log($"아이돌 '{idolCharacter.characterName}': '{scheduleNavType.targetNavPoint.pointName}'로의 이동이 완료되었거나 중단되었습니다 (도착 콜백 미호출).");
                    }
                }
                else
                {
